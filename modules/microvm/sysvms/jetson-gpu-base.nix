...
#
# Jetson GPU Guest Base Module
#
# Minimal shared base for an Orin AGX guest that owns the passed-through GPU.
# The concrete VM identity comes from hostConfig, allowing gpu-vm and
# sensing-vm to share the hardware/runtime foundation without sharing policy.
#
...
# ghaf.hardware.definition.<owner>.extraModules and applied through
# applyVmConfig at evaluatedConfig time.
#
# Usage in profiles:
#   lib.nixosSystem {
#     modules = [ inputs.self.nixosModules.<owner>-base ];
...
in
{
  ...
  ghaf = {
    ...
    # No dedicated GIVC role exists for GPU-owning guests. Enable transport
    # without a policy client; add a role only when the VM needs commands.
    givc = {
  ...
  systemd.services."${vmName}-node-access" = {
