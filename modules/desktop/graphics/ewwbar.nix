  ...
  config = lib.mkIf (cfg.enable && config.ghaf.givc.enable) {
      ...
      text = ''
        ...
        (box
            ...
            (widget_button
                    ...
                    :icon "${logout-icon}"
                    :title "Log Out"
                    :onclick "${eww-popup}/bin/eww-popup power-menu & ${ghaf-session-ctrl}/bin/ghaf-session-ctrl logout &")
            (widget_button
                    :class "power-menu-button"
                    :icon "${lock-icon}"
                    :title "Lock"
                    :onclick "${eww-popup}/bin/eww-popup power-menu & ${ghaf-session-ctrl}/bin/ghaf-session-ctrl lock &")))
    ...
    environment.etc."eww/eww.scss" = {
      text = ''
        ...
        .tray menu {
            @include floating_widget($margin: 0, $unset: false);
            font-family: Inter;
            font-size: 1.1em;
            background-color: $bg-primary;
